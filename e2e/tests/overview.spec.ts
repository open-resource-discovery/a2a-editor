...
test.describe("Agent Card Overview", () => {
  ...
  test("should show agent description", async ({ playground }) => {
    await expect(playground.agentDescription).toContainText("echo agent", { ignoreCase: true });
    // The description comes from the mock agent card after connection
    await expect(playground.agentDescription).toContainText("testing connectivity");
  });
